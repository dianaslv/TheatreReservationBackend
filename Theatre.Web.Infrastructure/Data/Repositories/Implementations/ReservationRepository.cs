using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using Theatre.Web.Core.Extensions;
using Theatre.Web.Core.Helpers.Commons;
using Theatre.Web.Core.Helpers.Commons.Filters.Interfaces;
using Theatre.Web.Core.Models.Entities;
using Theatre.Web.Core.Repository.Interfaces;
using Theatre.Web.Infrastructure.Data.Context;
using Theatre.Web.Infrastructure.IOC;

namespace Theatre.Web.Infrastructure.Data.Repositories.Implementations
{
    [RegistrationKind(Type = RegistrationType.Scoped)]
    public class ReservationRepository : IReservationRepository
    {
        private readonly DataContext m_dataContext;

        public ReservationRepository(DataContext dataContext)
        {
            m_dataContext = dataContext;
        }

        public async Task CreateAsync(Reservation entity)
        {
            entity.CreatedAt = DateTime.UtcNow;
            await m_dataContext.Reservations.AddAsync(entity);
            await m_dataContext.SaveChangesAsync();
        }

        public async Task UpdateAsync(List<Reservation> entities)
        {
            m_dataContext.Reservations.UpdateRange(entities);
            await m_dataContext.SaveChangesAsync();
        }

        public async Task DeleteAsync(Reservation entity)
        {
            m_dataContext.Reservations.Remove(entity);
            await m_dataContext.SaveChangesAsync();
        }
        
        public async Task<Tuple<int, List<Reservation>>> SearchAsync(Pagination pagination, IFilter<Reservation> filtering)
        {
            return await filtering
                .Filter(m_dataContext.Reservations.AsQueryable())
                .WithPaginationAsync(pagination);
        }
    }
}